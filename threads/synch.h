...
class Lock {
  ...
  private:
    ...
    Thread* owner; //Current thread using the lock
    Semaphore* semaphore; //Semaphore needed to make lock
...
class Condition {
  ...
  private:
    char* name; //Used in debugging
    Semaphore *semaphore; //Threads waiting to use this condition var
    // plus some other stuff you'll need to define
    Semaphore *mutex; //mutex for whenw e modify count
    int count; // how many threads are waiting on the mutex
};
#endif // SYNCH_H
